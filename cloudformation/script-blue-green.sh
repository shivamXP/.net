...
sudo aws cloudformation create-stack --stack-name EcsService-$commit --template-body file://cloudformation/service.yaml --parameters=file://cloudformation/ServiceParameters.json  --capabilities CAPABILITY_IAM --profile saml --region us-east-1
cd ..
rm -rf .net